"""
Bank integration and reconciliation module
"""
import difflib
import logging
from database import get_db_connection
import traceback

logger = logging.getLogger(__name__)

class BankIntegration:
    def __init__(self):
        self.match_threshold = 0.8  # 80% similarity for matching
    
    def import_bank_transactions(self, bank_account_id, transactions_data):
        """Import transactions from bank"""
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            
            for tx in transactions_data:
                cursor.execute('''
                    INSERT OR IGNORE INTO bank_transactions
                    (bank_account_id, external_transaction_id, date, description, amount, currency_code)
                    VALUES (?, ?, ?, ?, ?, ?)
                ''', (
                    bank_account_id,
                    tx.get('external_id'),
                    tx.get('date'),
                    tx.get('description'),
                    tx.get('amount'),
                    tx.get('currency', 'PKR')
                ))
            
            conn.commit()
            conn.close()
            logger.info(f"Successfully imported {len(transactions_data)} bank transactions")
        except Exception as e:
            logger.error(f"Error importing bank transactions: {str(e)}\n{traceback.format_exc()}")
            raise e
    
    def find_matches(self, bank_tx_id):
        """Find potential matches for a bank transaction"""
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            
            # Get the bank transaction
            cursor.execute('SELECT * FROM bank_transactions WHERE id = ?', (bank_tx_id,))
            bank_tx = cursor.fetchone()
            
            if not bank_tx:
                logger.warning(f"Bank transaction with ID {bank_tx_id} not found")
                return []
            
            # Find potential matches in our transactions
            cursor.execute('''
                SELECT * FROM transactions 
                WHERE date = ? AND ABS(amount - ?) < 1.0  -- Small tolerance for rounding
            ''', (bank_tx['date'], bank_tx['amount']))
            
            potential_matches = cursor.fetchall()
            
            matches = []
            for tx in potential_matches:
                similarity = difflib.SequenceMatcher(None, bank_tx['description'], tx['description']).ratio()
                if similarity >= self.match_threshold:
                    matches.append({
                        'transaction': tx,
                        'similarity': similarity
                    })
            
            conn.close()
            logger.info(f"Found {len(matches)} potential matches for bank transaction {bank_tx_id}")
            return sorted(matches, key=lambda x: x['similarity'], reverse=True)
        except Exception as e:
            logger.error(f"Error finding matches for bank transaction {bank_tx_id}: {str(e)}\n{traceback.format_exc()}")
            raise e
    
    def reconcile_transaction(self, bank_tx_id, internal_tx_id):
        """Reconcile a bank transaction with an internal transaction"""
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            
            cursor.execute('''
                UPDATE bank_transactions 
                SET status = 'reconciled', matched_transaction_id = ?
                WHERE id = ?
            ''', (internal_tx_id, bank_tx_id))
            
            if cursor.rowcount == 0:
                logger.warning(f"No bank transaction found with ID {bank_tx_id} for reconciliation")
                raise ValueError(f"Bank transaction with ID {bank_tx_id} not found")
            
            conn.commit()
            conn.close()
            logger.info(f"Successfully reconciled bank transaction {bank_tx_id} with internal transaction {internal_tx_id}")
        except Exception as e:
            logger.error(f"Error reconciling bank transaction {bank_tx_id} with internal transaction {internal_tx_id}: {str(e)}\n{traceback.format_exc()}")
            raise e